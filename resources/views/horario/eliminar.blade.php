...
@section('subcontent')
{{-- <form action="{{ route('apoderado.show') }}" method="GET" id="searchForm">
        ...
        @else
              ...
              <tr class="bg-white border-b">
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  {{ $itemgrado->grado }}{{$itemgrado->nivel}}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    <form action="{{ route('horario.destroyByGrado', $itemgrado->id_grado) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE') <!-- This tells Laravel to treat it as a DELETE request -->
                        <button type="submit" class="relative h-10 max-h-[40px] w-10 max-w-[40px] 
          ...
          <div class="mt-4">
            {{ $grado->links() }} <!-- Esto genera los enlaces de paginación -->
        </div>
        </div>
      </div>
    </div>
...
<script>
    let deleteId = null; // Variable para almacenar el ID del grado a eliminar

    // Función para abrir el modal de confirmación
    function openDeleteModal(id) {
        deleteId = id; // Asignamos el ID del grado a eliminar
        document.getElementById('confirmationModal').classList.remove('hidden'); // Mostramos el modal
    }
    ...
    document.getElementById('confirmButton').onclick = function () {
        if (deleteId !== null) {
            // Realizamos la petición de eliminación mediante una solicitud AJAX
            window.location.href = `/horario/eliminar/${deleteId}`;
        }
    };
</script>
