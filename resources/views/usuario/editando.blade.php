...
<section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
    ...
    {{-- {{ route('usuario.update', $usuario->usu_id) }} --}}
    <form id="usuarioedit" action="{{route('usuario.update', $usuario->usu_id) }}" method="POST">
        @csrf
    @method('PUT') 
    ...
    ondblclick="return false;">
        ...
        <div class="flex justify-end mt-6">
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <a href="{{route('usuario.show')}}" class="px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600" >Regresar</a>
...
<script>
    ...
    const confirmButton = document.getElementById('confirmButton');
     ...
     confirmButton.addEventListener('click', () => {
        ...
        document.getElementById('usuarioedit').submit();
    });
...
<script>
    ...
    editPasswordBtn.addEventListener('click', function() {
        passwordInput.disabled = false;   // Habilitar el campo de contraseña
        passwordInput.value = '';         // Limpiar el valor actual para ingresar una nueva contraseña

        // Eliminar restricciones para escribir y copiar
        passwordInput.removeAttribute('style');  // Quita el estilo que impide la selección del texto
        passwordInput.onmousedown = null;        // Permitir el uso del mouse
        passwordInput.onselectstart = null;      // Permitir la selección de texto
        passwordInput.oncopy = null;             // Permitir copiar
        passwordInput.oncut = null;              // Permitir cortar
        passwordInput.onpaste = null;            // Permitir pegar
        passwordInput.oncontextmenu = null;      // Permitir el menú contextual (clic derecho)
        passwordInput.ondblclick = null;         // Permitir el doble clic
    });
</script>
@endsection