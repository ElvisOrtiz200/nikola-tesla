...
@section('subcontent')
<form action="{{ route('docente.show') }}" method="GET" id="searchForm">
...
<div class="flex flex-col">
    <div class="overflow-x-auto sm:mx-0.5 lg:mx-0.5">
      <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
        ...
        @else
              @foreach ($docente as $docenteUser)
              <tr class="bg-white border-b">
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{  $docenteUser->recursoshh->per_dni  }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  {{ $docenteUser->doc_especialidad }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $docenteUser->doc_nivel_educativo }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $docenteUser->doc_fecha_nac }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $docenteUser->doc_estado }}
                </td>
                {{-- {{ route('docente.editando', $docenteUser->apo_dni) }} --}}
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  <a href="{{ route('docente.editando', $docenteUser->doc_id) }} " 
          ...
          <div class="mt-4">
            {{ $docente->links() }} <!-- Esto genera los enlaces de paginación -->
        </div>
        </div>
      </div>
    </div>
