...
<section class="max-w-4xl p-6 mx-auto bg-white rounded-md drop-shadow-2xl dark:bg-gray-800">
    <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Registrar curso</h2>

    <form id="rolCrear" action="{{route('curso.store')}}" method="POST" onsubmit="return validarFormulario()">
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
            <div>
                <label class="text-gray-700 dark:text-gray-200" for="acu_nombre">Nombre del curso</label>
                <input id="acu_nombre" name="acu_nombre" type="text" 
            ...
            <script>
                function validarFormulario() {
                    ...
                    // Validación del Nombre del curso (campo obligatorio)
                    var acuNombre = document.getElementById('acu_nombre').value;
                    if (!acuNombre.trim()) {
            ...
            <div class="max-w-xs mx-auto mt-4">
                <label class="text-gray-700 dark:text-gray-200" for="acu_nombre">Seleccionar grado académico</label>
                ...
                <option value="{{ $itemGrados->id_grado}}">{{ $itemGrados->grado_nombre }} - {{ $itemGrados->nivel_nombre }}</option>
            @endforeach
