...
<section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
    ...
    <form id="roledit" action="{{ route('aula.destroy', $aulas->id_aula) }}" method="POST">
        @csrf
    @method('DELETE')
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
            ...
            <div class="relative group">
                <button type="button" id="dropdown-button" class="inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500">
                    <span class="mr-2">{{ $aulas->grado->nombre }}</span>
        ...
        <div class="flex justify-end mt-6">
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <a href="{{route('aula.eliminar')}}" class="px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600" >Regresar</a>
