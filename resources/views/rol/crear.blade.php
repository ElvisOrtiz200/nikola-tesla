...
<section class="max-w-4xl p-6 mx-auto bg-white rounded-md drop-shadow-2xl dark:bg-gray-800">
    <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Registrar rol</h2>

    <form id="rolCrear" action="{{route('rol.store')}}" method="POST" onsubmit="return validarFormulario()">
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
            <div>
                <label class="text-gray-700 dark:text-gray-200" for="nombre_rol">Nombre del rol</label>
                <input id="nombre_rol" name="nombre_rol" type="text" 
        ...
        <div id="errorDiv" class="text-red-500 hidden mt-2"></div>
    ...
    function validarFormulario() {
        ...
        if (!regex.test(nombreRol)) {
            errorDiv.textContent = "Solo se permiten letras y espacios en el nombre del rol."; 
            errorDiv.classList.remove('hidden'); 
            return false; 
        }
        return true; 
