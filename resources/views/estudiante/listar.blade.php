...
@section('subcontent')
<form action="{{ route('estudiante.showListar') }}" method="GET" id="searchForm">
      ...
      <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
        ...
        @else
              ...
              <tr class="bg-white border-b">
                ...
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $estudiante->alu_direccion }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $estudiante->alu_telefono }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ $estudiante->alu_estado }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  <a href="{{ route('estudiante.editando', $estudiante->alu_dni) }}" 
          ...
          <div class="mt-4">
            {{ $estudiantes->links() }} <!-- Esto genera los enlaces de paginación -->
        </div>
        </div>
      </div>
    </div>
